...
<body>
    <?php
       ...
       $sel=$data->prepare("SELECT * FROM faq order By DateF DESC ");
       $sel->execute();
       $c=$sel->rowCount();
       if(isset($_POST['supprimere'])){
        $id=$_POST['supprimere'];
        $rol=$data->prepare("DELETE FROM faq WHERE idF='$id'");
        $rol->execute();
    ...
    $R=$_POST['rep'];
    $date=$_POST['date'];
    $creat=$data->prepare("INSERT INTO  faq (`idF`, `QesF`, `RepF`, `DateF`) VALUES (NULL, '$Q', '$R', '$date');");
    $creat->execute();
    if($creat){
        $dex="block";
        $sec = "2.3";
        header("Refresh:$sec; url=faqAdmin.php");
    }
    }
    if(isset($_POST["update"])){
        ...
        $R=$_POST['rep'];
        $date=$_POST['date'];
    $ce=$data->prepare("UPDATE faq set QesF='$Q',RepF='$R' Where idF=$id");
    $ce->execute();
    header('Location:faqAdmin.php');
    }
...
<div class="row mt-3">
    <div class="col-md py-2"><label>Id FAQ</label><input type="text" id="id" name="id" class="form-control p-2" readonly></div>
    <div class="col-md py-2"><label>Qeuestion FAQ</label><input type="text" id="qeu" name="qes" class="form-control  p-2"></div>
</div>
<div class="row ">
<div class="col-md py-2"><label>Repanse FAQ</label><input type="text" id="rep" name="rep" class="form-control  p-2"></div>
    <div class="col-md py-2"><label>Date FAQ</label><input  name="date"  value='<?php echo date('Y-m-d') ?>' type="date" class="form-control  p-2"></div>
</div>
...
<div style="overflow-x:auto;">
      ...
      <form method='POST'>
           ...
           foreach($sel as $me){
               ...
               <tr >
               <td>".$me['idF']."</td>
               <td><textarea disabled>".$me['QesF']."</textarea></td>
               <td><textarea disabled>".$me['RepF']."</textarea></td>
               <td>".$me['DateF']."</td>
                <td><button  class='btn' type='button' onclick='app(this)' value='".$me['idF']."'><i class='bi bi-pencil-square text-success' style='font-size:22px;'></i></button></td>
                <td><button  class='btn'   onclick='supprimer(this)'   name='supprimere' type='button'  value='".$me['idF']."'><i class='bi bi-trash3-fill text-danger' style='font-size:22px;'></i></button></td>
               </tr>
               ";
           }
